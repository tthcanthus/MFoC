...
############################################################
#######     mixture of scalar-on-composition   #############
############################################################
...
W0 <- newW[[1]]
ll <- newW$ll
#### E step and M step for MFoS with independent covariance structure 
while (abs(diff) > epsilon && iter < max_iter && restart < max_restart) { 
  
  MResult1 = MUpdate(Y.list,W0,h1)
  newW1 = EUpdate(Y.list,MResult1$coef, MResult1$sigma, MResult1$Pai0)
  newLL = newW1$ll
...
#### E step and M step for MFoS with correlation structure 
